...
Page({
  ...
  fetchData: function () {
    let authorityId = this.data.authorityId
    wx.showLoading({
      title: '加载中',
      mask: true
    });
    var that = this
    wx.request({
      url: `${URL}/team/getBySecondAuthority?authorityId=${authorityId}`,
      success(res) {
  ...
  gotoEditTeam: function (e) {
    ...
    wx.navigateTo({
      url: '/pages/management/team_edit/team_edit?id=' + id,
    })
  },

  createNewTeam() {
    wx.navigateTo({
      url: '/pages/management/team_new/team_new?authorityLevel=2&authorityId=' + this.data.authorityId,
    })
  },
